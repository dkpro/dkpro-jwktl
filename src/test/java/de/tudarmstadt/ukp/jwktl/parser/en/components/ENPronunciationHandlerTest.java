...
//		printPronunciations(page);
	...
	public void testWorker() {
		...
		w.processBody("* {{a|UK}} {{IPA|en|/ˈdɪkʃən(ə)ɹi/}}", null);
		w.processBody("* {{a|North America}} {{enPR|dĭk'shə-nĕr-ē}}, {{IPA|en|/ˈdɪkʃənɛɹi/}}", null);
		w.processBody("* {{audio|en|en-us-dictionary.ogg|Audio (US)}}", null);
		w.processBody("* {{audio|en|en-uk-dictionary.ogg|Audio (UK)}}", null);

		w.processBody("* {{IPA|en|  /fɹiː/}}", null);
		w.processBody("* {{audio|en-us-free.ogg|Audio (US)|lang=en}}", null);
		w.processBody("* {{audio|En-uk-free.ogg|Audio (UK)|lang=en}}", null);
		w.processBody("*: {{rhymes|en|iː}}", null);

		w.processBody("* {{a|RP}} {{IPA|en|/dɒɡ/}}", null);
		w.processBody("* {{a|US}} {{IPA|en|/dɔɡ/}}", null);
		w.processBody("* {{a|US, in accents with the [[cot-caught merger]]}} {{IPA|en|/dɑɡ/}}", null);
		w.processBody("* {{audio|en|en-us-dog.ogg|Audio (US)}}", null);
		w.processBody("* {{audio|en|En-uk-a dog.ogg|Audio (UK)}}", null);
		w.processBody("*: {{rhymes|en| ɒɡ}}", null);

		w.processBody("* {{IPA|es|/pje/}}", null);

		w.processBody("* {{IPA|mul|/ɑː/|/a/}} {{qualifier|most languages}}", null);

		w.processBody("* {{qualifier|letter name}}", null);
		w.processBody("** {{a|RP|GenAm}} {{IPA|en|/eɪ̯/}}", null);
		w.processBody("** {{audio|en|en-us-a.ogg|Audio (US)}}", null);
		w.processBody("** {{a|AusE}} {{IPA|en|/æɪ/}}", null);
		w.processBody("* {{rhymes|en|eɪ}}", null);
		...
		w.processBody("* {{sense|letter name}} {{IPA|eo|/a/}}", null);
		w.processBody("* {{sense|phoneme}} {{IPA|eo|/a/}}", null);
	...
	public void testRawPronunciationType() {
		final List<IPronunciation> pronunciations = process("* {{IPA|/budɛ̃/|lang=fr}}",
				"* {{fr-IPA|cauboille}}", "* {{zh-pron}}", "* {{fi-pronunciation}}");
		assertEquals(4, pronunciations.size());
		...
		assertEquals("{{fi-pronunciation}}", pronunciations.get(3).getText());
		assertEquals(PronunciationType.RAW, pronunciations.get(3).getType());
	}
